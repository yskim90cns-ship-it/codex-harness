...
if (-not (Test-Path $nextCommand)) {
  throw "Next.js was not installed correctly. Delete node_modules and run this file again."
}

Write-Host "Building production app..." -ForegroundColor Cyan
Write-Host "This step can use more memory than the normal Windows start script." -ForegroundColor DarkGray
$env:NODE_OPTIONS = "--max-old-space-size=4096"
npm.cmd run build

Write-Host ""
Write-Host "Starting production server. Keep this window open while using the app." -ForegroundColor Green
